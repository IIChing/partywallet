...
function Controller(){  
    ...
    launch: function(){
        ...
        me.counterparty   = QB.app.getController('Counterparty');
        ...
        QB.PASSCODE       = 0000;                           // Default passcode used to encrypt wallet
        QB.WALLET_HEX     = null;                           // HD wallet Hex key
        QB.WALLET_KEYS    = {};                             // Object containing of address/private keys
        QB.WALLET_NETWORK = sm.getItem('network') || 1;     // (1=Mainnet, 2=Testnet)
        QB.WALLET_PREFIX  = sm.getItem('prefix')  || null;  // 4-char wallet hex prefix (used to quickly find addresses associated with this wallet in datastore)
        QB.WALLET_ADDRESS = sm.getItem('address') || null;  // Current wallet address info
        QB.TOUCHID        = sm.getItem('touchid') || false; // TouchID Authentication enabled (iOS 8+)
        QB.NETWORK_INFO   = {};                             // latest network information (price, fees, unconfirmed tx, etc)
        QB.API_KEYS       = {
        ...
        if(wall){
            ...
            var successFn = function(pass){
                if(pass)
                    QB.WALLET_PASSCODE = pass;
                me.decryptWallet();
                me.setWalletNetwork(QB.WALLET_NETWORK);
                me.setWalletAddress(QB.WALLET_ADDRESS, true);
                me.showMainView();
                ...
                if(network)
                    QB.NETWORK_INFO = Ext.decode(network);
                // Parse in last known network fees
                if(QB.NETWORK_INFO.fee_info){
                    var o = QB.NETWORK_INFO.fee_info;
                    QB.MINER_FEES.medium = o.low_priority;
                    QB.MINER_FEES.fast   = o.optimal;
                }
    ...
    showWelcomeView:      function(){ this.showView('welcomeView','QB.view.Welcome');  },
    showMainView:         function(){ this.showView('mainView','QB.view.Main'); },
    showAddressListView:  function(){ this.showView('addressList','QB.view.AddressList'); },
    showQRCodeView:       function(cfg){ this.showView('qrcodeView','QB.view.QRCode', cfg); },
    showScanQRCodeView:   function(cfg){ this.showView(null,'QB.view.Scan',cfg); },
    showPasscodeView:     function(cfg){ this.showView('passcodeView','QB.view.Passcode', cfg); },
    showPassphraseView:   function(cfg){ this.showView('passphraseView','QB.view.Passphrase', cfg); },
    showCallbackView:     function(cfg){ this.showView('callbackView','QB.view.Callback', cfg); },
    ...
    generateWallet: function(phrase, callback){
        ...
        QB.WALLET_PREFIX  = String(h.substr(0,5));
        sm.setItem('prefix', QB.WALLET_PREFIX);
        ...
        var addr = me.getFirstWalletAddress(QB.WALLET_NETWORK);
        if(addr)
    ...
    showWalletPassphrase: function(){
        var me = this,
            m  = Mnemonic.fromHex(QB.WALLET_HEX),
    ...
    encryptWallet: function(){
        ...
        var enc = CryptoJS.AES.encrypt(QB.WALLET_HEX, String(QB.PASSCODE)).toString();
        sm.setItem('wallet', enc);
        // Encrypt any imported private keys
        var enc = CryptoJS.AES.encrypt(Ext.encode(QB.WALLET_KEYS), String(QB.PASSCODE)).toString();
    ...
    decryptWallet: function(){
        ...
        // Decrypt wallet
        if(w){
            var dec = CryptoJS.AES.decrypt(w, String(QB.PASSCODE)).toString(CryptoJS.enc.Utf8);
            QB.WALLET_HEX = dec;
        }
        // Decrypt any saved/imported private keys
        if(p){
            var dec = CryptoJS.AES.decrypt(p, String(QB.PASSCODE)).toString(CryptoJS.enc.Utf8);
            QB.WALLET_KEYS = Ext.decode(dec);
        }
    },
    ...
    addWalletPrivkey: function(key, alert){
        ...
        var me      = this,
            ...
            n       = (QB.WALLET_NETWORK==2) ? 'testnet' : 'mainnet',
        ...
        if(address){
            var rec = store.add({
                id: QB.WALLET_PREFIX + '-' + QB.WALLET_NETWORK + '-' + String(address).substring(0,4),
                index: 9999,
                prefix: QB.WALLET_PREFIX,
                network: QB.WALLET_NETWORK,
                address: address,
        ...
        if(QB.WALLET_KEYS){
            console.log('QB.WALLET_KEYS=',QB.WALLET_KEYS);
            QB.WALLET_KEYS[address] = key;
            me.encryptWallet();
        }
    ...
    addWalletAddress: function(count, network, force, alert){
        var me       = this,
            addr     = null,
            network  = (network) ? network : QB.WALLET_NETWORK,
            bc       = bitcore,
            n        = (network==2) ? 'testnet' : 'mainnet',
            force    = (force) ? true : false,
            net      = bc.Networks[n],
            key      = bc.HDPrivateKey.fromSeed(QB.WALLET_HEX, net);   // HD Private key object
        ...
        for( var i = 0; total<count; i++){
            ...
            if(!found){
                if(force)
                    total++;
                var rec = store.add({
                    id: QB.WALLET_PREFIX + '-' + network + '-' + (i+1),
                    index: i,
                    prefix: QB.WALLET_PREFIX,
                    network: network,
                    address: address,
    ...
    setWalletAddressLabel: function(val, addr){
        var me    = this,
            sm    = localStorage,
            store = Ext.getStore('Addresses'),
            addr  = (addr) ? addr : QB.WALLET_ADDRESS.address;
        // Remove any filters on the store so we are dealing with all the data
        store.clearFilter();
        // Locate address and update stored label value
        store.each(function(rec){
            var o = rec.data;
            if(o.network==QB.WALLET_NETWORK && o.prefix==QB.WALLET_PREFIX && o.address==addr){
                QB.WALLET_ADDRESS.label = val;
                o.label = val;
                rec.setDirty(true);
                return false;
            }
    ...
    getFirstWalletAddress: function(network){
        ...
        store.each(function(rec){
            var o = rec.data;
            if(o.network==network && o.prefix==QB.WALLET_PREFIX && o.index==0){
                addr = o.address;
                return false;
            }
        });
    ...
    getCurrencyPrice: function(currency, type){
        var value = false;
        Ext.each(QB.NETWORK_INFO.currency_info, function(item){
            if(item.id==currency){
                if(type=='usd')
    ...
    getAddressBalances: function(address, callback){
        var me     = this,
            addr   = (address) ? address : QB.WALLET_ADDRESS.address,
            prefix = addr.substr(0,5),
            store  = Ext.getStore('Balances'),
            net    = (QB.WALLET_NETWORK==2) ? 'tbtc' : 'btc',
            hostA  = (QB.WALLET_NETWORK==2) ? 'tbtc.blockr.io' : 'btc.blockr.io',
            hostB  = (QB.WALLET_NETWORK==2) ? 'testnet.xchain.io' : 'xchain.io';
        // Get Address balance from blocktrail
        me.ajaxRequest({
            url: 'https://api.blocktrail.com/v1/' + net + '/address/' + address + '?api_key=' + QB.API_KEYS.BLOCKTRAIL,
            success: function(o){
                if(o.address){
    ...
    updateAddressBalance: function(address, type, asset, asset_longname, quantity, estimated_value){
        ...
        var me     = this,
            addr   = (address) ? address : QB.WALLET_ADDRESS,
            prefix = addr.substr(0,5),
            store  = Ext.getStore('Balances');
            record = store.add({
                id: prefix + '-' + asset,
    ...
    promptAddressPrivkey: function(callback){
        var me  = this,
            bc  = bitcore,
            net = (QB.WALLET_NETWORK==2) ? 'testnet' : 'mainnet';
        Ext.Msg.show({
    ...
    setPasscode: function(code){
        var me = this,
            sm = localStorage;
        if(code){
            QB.WALLET_PASSCODE = code;
            var enc = CryptoJS.AES.encrypt(String(QB.WALLET_PASSCODE), String(QB.WALLET_PASSCODE)).toString();
    ...
    getTransactionHistory: function(address, callback){
        ...
        me.ajaxRequest({
            url: 'https://api.blocktrail.com/v1/' + net + '/address/' + address + '/transactions?limit=100&sort_dir=desc&api_key=' + QB.API_KEYS.BLOCKTRAIL,
            success: function(o){
    ...
    showMainMenu: function(side){
        ...
        if(!me.menu)
            me.menu = Ext.create('QB.view.MainMenu');
    ...
    getPrivateKey: function(network, address){
        ...
        var priv = QB.WALLET_KEYS[address];
        ...
        if(o.action){
            ...
            if(o.action=='sign'){
                if(o.callback){
                    ...
                    var addr = (o.address) ? o.address : QB.WALLET_ADDRESS.address,
                        host = me.getUrlHostname(o.callback),
                        key  = me.getPrivateKey(QB.WALLET_NETWORK, addr);
                    // Only proceed if we were able to get the key for the address
                    if(key){
                        var sig = me.signMessage(QB.WALLET_NETWORK, addr, o.message);
                        if(sig){
                            o.address   = addr;
                            o.signature = sig;
    ...
    getBalance: function(asset){
        var balances = Ext.getStore('Balances'),
            balance  = 0,
            prefix   = QB.WALLET_ADDRESS.address.substr(0,5);
    ...
    broadcastTransaction: function(network, tx, callback){
        var me  = this,
            ...
            host = (QB.WALLET_NETWORK==2) ? 'testnet.xchain.io' : 'xchain.io',
        ...
        me.ajaxRequest({
            ...
            success: function(o){
                if(o && o.currency_info){
                    ...
                    QB.MINER_FEES = Ext.apply(QB.MINER_FEES,{       
